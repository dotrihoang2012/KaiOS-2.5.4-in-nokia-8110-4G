        ...
        if not skipresponse:
            ...
            try:
                if b"raw hex token" in rdata:
                    rdata = rdata
                # Extract only the XML <response> part - strip leading binary data
                xml_rdata = rdata
                if b"<response" in rdata:
                    start = rdata.find(b"<response")
                    xml_start = rdata.rfind(b"<?xml", 0, start)
                    if xml_start != -1:
                        xml_rdata = rdata[xml_start:]
                    else:
                        xml_rdata = rdata[start:]
                try:
                    resp = self.xml.getresponse(xml_rdata)
                except:
                    try:
                        decoded = rdata.decode('utf-8', errors='replace')
                        if "<response" in decoded:
                            idx = decoded.find("<response")
                            xml_start = decoded.rfind("<?xml", 0, idx)
                            decoded = decoded[xml_start if xml_start != -1 else idx:]
                        resp = self.xml.getresponse(bytes(decoded, 'utf-8'))
                    except:
                        resp = {"value": "ACK"}
                ...
                try:
                    self.log.debug("Error on getting xml response:" + rdata.decode('utf-8', errors='replace'))
                except:
                    self.log.debug("Error on getting xml response (binary data)")
        ...
        if rsp[0]:
            if "value" in rsp[1]:
                if rsp[1]["value"] == "NAK":
                    if display:
                        try:
                            self.log.error(rsp[2].decode('utf-8', errors='replace') if isinstance(rsp[2], (bytes, bytearray)) else str(rsp[2]))
                        except:
                            self.log.error("NAK received")
                    return b""
            bytesToRead = self.cfg.SECTOR_SIZE_IN_BYTES * num_partition_sectors
            # Check if binary data already arrived together with the xmlsend response
            raw = rsp[2] if isinstance(rsp[2], (bytes, bytearray)) else b""
            # Strip trailing XML tags from raw data to get pure binary
            binary_end = len(raw)
            if b"<?xml" in raw:
                binary_end = raw.find(b"<?xml")
            elif b"<data>" in raw:
                binary_end = raw.find(b"<data>")
            prefetched = bytearray(raw[:binary_end])
            total = bytesToRead
            resData += prefetched
            bytesToRead -= len(prefetched)
            dataread = len(prefetched)
            old = 0
            prog = 0
            if display:
                print_progress(prog, 100, prefix='Progress:', suffix='Complete', bar_length=50)
            while bytesToRead > 0:
                tmp = self.cdc.read(self.cfg.MaxPayloadSizeToTargetInBytes)
                # Strip XML suffix if mixed in last chunk
                chunk_end = len(tmp)
                if b"<?xml" in tmp:
                    chunk_end = tmp.find(b"<?xml")
                elif b"<data>" in tmp:
                    chunk_end = tmp.find(b"<data>")
                resData += tmp[:chunk_end]
                bytesToRead -= len(tmp[:chunk_end])
                dataread += len(tmp[:chunk_end])
                if chunk_end < len(tmp):
                    break  # XML suffix found, data is complete
                prog = int(float(dataread) / float(total) * float(100))
                if prog > old:
                    if display:
                        print_progress(prog, 100, prefix='Progress:', suffix='Complete', bar_length=50)
